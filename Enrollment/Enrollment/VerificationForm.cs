...
namespace Enrollment
{
	...
	*/
	public class VerificationForm : CaptureForm
	{
		...
		protected override void Process(DPFP.Sample Sample)
		{
			...
			if (features != null)
			{
                ...
                con.Open();
                    ...
                    */
                    foreach(var emp in empdatabase.tbl_emp)
                    {
                        stream = new MemoryStream(emp.emp_finger);
                        ...
                        if (result.Verified)
                        {
                            ...
                            {
                                string query = "insert into tbl_attendance(empid,val,today_date,month,year)values((select Id from tbl_emp where emp_name='" + emp.emp_name + "'),'" + val + "','" + todaydate + "','" + thismonth + "','" + year + "')";
                                SqlCommand cmd = new SqlCommand(query, con);
                                con.Open();
                                ...
                                if (a > 0)
                                {
                                    ...
                                    MessageBox.Show("Attendance is taken Successfully for " + emp.emp_name, "Attendance Capture");
        ...
        private int ifalreadytoday(string empname,string todaydate)
        {
            int a = 0;
        ...
        {
            string st = @"Data Source=161.97.77.250;Initial Catalog=empdatabase;user id=user1;password=1111";
